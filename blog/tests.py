...
class PortfolioSnapshotLockTests(TestCase):
    def auth_header(self):
        credentials = base64.b64encode(b'hashen:hashen123').decode('ascii')
    ...
    def test_month_end_lock_updates_current_mtd_snapshot(self, localdate, capture_prices):
        ...
        snapshot_response = self.client.get('/api/portfolio-snapshot/', **self.auth_header())
        self.assertEqual(snapshot_response.status_code, 200)
        snapshot_payload = snapshot_response.json()
        self.assertFalse(snapshot_payload['pending_month_end'])

        progression_response = self.client.get('/api/wealth-progression/', **self.auth_header())
